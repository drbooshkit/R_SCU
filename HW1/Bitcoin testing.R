...
getSymbols(c("USO"))
# Create a data frame of adjusted close prices
uso <- data.frame(USO[,6])
# calculate daily retuns and add that to Data Frame
n <- length(uso$USO.Adjusted)
dailyRets4 <- log(uso$USO.Adjusted[2:n]/uso$USO.Adjusted[1:(n-1)])
dailyRets4[2:n] <- dailyRets4[1:(n-1)]
dailyRets4[1] <- 0.0
data4 <- cbind(uso, dailyRets4)
...
total <- rename(total, c(Weighted.Price="BTC", 
  dailyRets="BTCrets",GOOG.Adjusted="GOOG",dailyRets2="GOOGrets", 
  GLD.Adjusted="GLD",dailyRets3="GLDrets", USO.Adjusted="USO", 
  dailyRets4="USOrets"))
...
legend("right", inset=.05, title="Stocks", c("BTC","GOOG","GLD","USO"),
...
x <- total$Date
y <- total$BTCrets*100
y2 <- total$GOOGrets*100
y3 <- total$GLDrets*100
y4 <- total$USOrets*100
main <- "Bitcoin-Equity Time Series"
xlab <- "Date"
ylab <- "Daily Change, %"
...
# Show correlation
rets <- data.frame(total$BTCrets, total$GOOGrets, total$GLDrets, total$USOrets)
y <- c("BTC", "GOOG", "GLD", "USO")
pairs(rets)
