...
ufo <- read.delim("e:/datamining/datasets/ufo_awesome.tsv",
                  sep = "\t",
...
# type using the 'strptime' function. but there is some malformed data
# But, something has gone wrong with the data. For now, we'll just ignore the errata
# by removing those entries that have not parsed correctly.  

# We know that the date # strings are always 8 characters long, and any deviation from this would indicate
# a row to ignore.  We will use the 'ifelse' function to construct a vector of Booleans indicating the problem rows
...
# the Year-Month of each incident.  Create by just referencing it
